window.onload = function() {
    ...
    // const updatebtn = document.getElementById("updateLink");
    const updatebtn = document.getElementById("searchinventorybtn");
    var httpRequest;
    ...
    updatebtn.addEventListener('click', function(element) {
        ...
        httpRequest = new XMLHttpRequest();
        const itemId = document.querySelector('#searchinput').value;
        // const qty = document.querySelector('').value;
        // const status = document.querySelector('').value;
    
        // var url = "http://localhost:8888/Latched-Ja.-Inventory-Management-System/scripts/items.php?itemid=" + itemId + "&context=" + status.toLowerCase();
        var url = "http://localhost:8888/Latched-Ja.-Inventory-Management-System/scripts/items.php?itemid=" + itemId;
        httpRequest.onreadystatechange = sendResult;
        httpRequest.open('GET', url);
        httpRequest.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        // httpRequest.send('id=' + encodeURIComponent(itemId) + '&qty=' + encodeURIComponent(qty) + '&status=' + encodeURIComponent(status.toLowerCase()));
        httpRequest.send('id=' + encodeURIComponent(itemId));
    });
